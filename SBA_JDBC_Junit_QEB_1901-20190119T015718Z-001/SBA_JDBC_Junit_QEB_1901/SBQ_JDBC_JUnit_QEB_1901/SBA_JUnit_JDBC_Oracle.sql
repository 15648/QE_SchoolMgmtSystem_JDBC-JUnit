...
CREATE OR REPLACE PROCEDURE deleteAttendingRecord(p_attending_id IN ATTENDING.attending_id%TYPE)
IS
BEGIN

  DELETE ATTENDING where attending_id = p_attending_id;

  COMMIT;

END;

CREATE OR REPLACE PROCEDURE deleteTeachingRecord(p_teaching_id IN TEACHING.teaching_id%TYPE)
IS
BEGIN

  DELETE TEACHING where teaching_id = p_teaching_id;
